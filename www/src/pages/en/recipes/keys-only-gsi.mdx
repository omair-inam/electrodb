...
const { data, cursor } = await assets.query.locations({ state: 'Georgia' }).go({ hydrate: true });
```
...
const { data, cursor } = await assets.query.locations({ state: 'Georgia' }).go({ ignoreOwnership: true, data: 'includeKeys' });
```
...
const result: any = await assets.query.locations({ state: 'Georgia' }).go({ data: 'raw' });
